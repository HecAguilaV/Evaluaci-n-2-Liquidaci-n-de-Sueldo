...
#Mientras el sueldo base y horas extras sean numeros positivos, se ejectua el codigo, al llevar los excepty, nos arrojara un mensaje que indique que estos valores sean positivos
    while True:
        try:
...
#Declaración de variables y funciones donde se realizaraán los cálculos correspondientes.
...
#Aqui se establece lo que se imprimira por pantalla.
#Se agregó a las variables numericas el formateador ".f2", que limita la cantidad de decimales hasta 2.
...
#En esta sección se genera el archivo ".txt" que contendrá la infomración mostrada anteriormente por pantalla.
...
def main():
    while True:
        ...
        #Aquí validamos si se desea generar otra liquidación, donde el usuario debe responder Si o No, con las letras "s" o "n".
        #Se implementa el metodo ".lower" para que se asuma que el caracter ingresado sea minúscula y asi evitar algún fallo en la lectura
        continuar = input("¿Desea calcular la liquidación de otro trabajador? (s/n): ").lower()
        if continuar != 's':
            break

#Código que me permite ejectuar y repetir el programa.
